...
Direktori ini berisi skrip pelatihan (*training scripts*) dan evaluasi (*evaluation scripts*) untuk mendeteksi apakah suatu kode sumber ditulis oleh **Manusia (Human-written)** atau **Kecerdasan Buatan (AI-generated)**. Deteksi pada model-model ini difokuskan **khusus untuk bahasa pemrograman Python**.
...
## 🚀 Cara Menjalankan Pelatihan & Evaluasi

### 1. Prasyarat (Requirements)
Pastikan dependensi Python berikut sudah terinstal di lingkungan kerja Anda:
```bash
pip install torch transformers datasets evaluate scikit-learn joblib scipy pandas numpy matplotlib
```
...
### 3. Melatih Model
* **Pelatihan Model ML Klasik**:
  ```bash
  python ml2.py
  ```
* **Pelatihan Model Deep Learning (CodeBERT)**:
  ```bash
  python codebert.py
  ```

### 4. Mengevaluasi Model (Menghasilkan Confusion Matrix & Laporan)
Untuk menguji performa model terlatih pada data uji (*test split* 20%) dan menghasilkan grafik Confusion Matrix serta Laporan Klasifikasi:

1. **Jalankan Evaluasi ML Klasik (TF-IDF + Logistic)**:
   ```bash
   python evaluate_ml2.py
   ```
2. **Jalankan Evaluasi Deep Learning (CodeBERT)**:
   ```bash
   python evaluate_codebert.py
   ```

Setelah dijalankan, skrip tersebut akan membuat folder **`metrics/`** di dalam folder `model/` yang berisi berkas output berikut:
* **`metrics/report_ml2.txt`** — Laporan klasifikasi (Precision, Recall, F1-Score) untuk model TF-IDF.
* **`metrics/report_codebert.txt`** — Laporan klasifikasi untuk model CodeBERT.
* **`metrics/confusion_matrix_ml2.png`** — Gambar grafik Confusion Matrix untuk model TF-IDF.
* **`metrics/confusion_matrix_codebert.png`** — Gambar grafik Confusion Matrix untuk model CodeBERT.
